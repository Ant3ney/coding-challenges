...
window.onload = function () {
   ...
   cardsParam
      ? deck.filter('id', cardsParam.split(' '))
      : console.log("Not sorting by card ID's");

   const suitsParam = urlParams.get('suits');
   isSuitsParamValid(suitsParam)
      ? deck.filter('suit', suitsParam.split(' '))
      : !suitsParam
      ? console.log('Suits param was not passed')
      : alert(`Query param ${suitsParam} is invalid`);

   const ranksParam = urlParams.get('ranks');
   isNumaricalParamsValid(ranksParam)
      ? deck.filter('rank', formatNumaricalParams(ranksParam))
      : !ranksParam
      ? console.log('Ranks param was not passed')
      : alert(`Query param ${ranksParam} is invalid`);

   const sortedParam = urlParams.get('sorted');
   sortedParam && sortedParam === 'true'
      ? deck.sort()
      : console.log('Not sorting cards');

   const limitParam = urlParams.get('limit');
   isNumaricalParamsValid(limitParam)
      ? deck.limit(limitParam)
      : !limitParam
      ? console.log('Limit param was not passed')
      : alert(`Query param ${limitParam} is invalid`);

   deck.drawFiltered();
};

function isNumaricalParamsValid(params) {
   if (!params) return false;
   let failedFlag = false;
   params.split(' ').forEach(param => {
      if (parseInt(param).toString() === 'NaN') {
         failedFlag = true;
      }
   });
...
function formatNumaricalParams(params) {
   return params.split(' ').map(param => parseInt(param));
}

function isSuitsParamValid(suits) {
   if (!suits) return false;
   let failedFlag = false;
   const validSuits = ['hearts', 'spades', 'diamonds', 'clubs'];
